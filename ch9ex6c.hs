...
res :: [Int] -> Int -> [Result]
res ns n = sortBy (\(_,x) (_,y) -> compare x y) [(e, abs(n - m)) | ns' <- choices ns, (e,m) <- results ns', m > 0]

solutions'' :: [Int] -> Int -> [Expr]
solutions'' ns n =
    let
        res' = res ns n
        firstVal = (snd . head) res'
        ord = length . values
    in
        sortBy (\x y -> compare (ord x) (ord y)) [e | (e,_) <- takeWhile (\(_,m) -> m == firstVal) res']
